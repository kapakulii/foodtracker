...
def upsert_metric(metric_date: str, data: DailyMetricUpdate, db: Session = Depends(get_db)):
    metric = db.query(DailyMetric).filter(DailyMetric.date == metric_date).first()
    if not metric:
        metric = DailyMetric(date=metric_date)
        db.add(metric)
